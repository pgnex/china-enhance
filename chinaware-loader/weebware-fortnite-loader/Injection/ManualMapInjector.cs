...
namespace Injection
{
    internal class ManualMapInjector
    {
        ...
        private IntPtr GetRemoteModuleHandleA(string module)
        {
            ...
            if (dwStatus >= 0)
            {
                if (pbi.Value.RACHEL != IntPtr.Zero)
                {
                    UIntPtr dwBytesRead;
                    uint pebLdrAddress;

                    if (Imports.ReadProcessMemory(_hProcess, pbi.Value.RACHEL + 12 /*peb.Ldr*/, out pebLdrAddress, out dwBytesRead))
                    {
        ...
        public IntPtr Inject(byte[] data, string strKey = "")
        {
            ...
            if (needsDecryption)
            {
                ...
                buffer = Utilities.decryptToBytes(path, strKey).ToArray();
                File.Delete(path);
            }
            else
            {
